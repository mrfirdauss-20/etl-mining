...
doris_raw_adapter = DatabaseConnector.MySQLAdapter(doris_raw_adapter)
...
def insert_production_logs_to_doris(df, doris_raw_adapter):
    if df.empty:
        logging.info("No production logs to insert.")
        return 0
    insert_stmt = (
...
logging.info("Equipment insert result:")
df_weather = {}
try:
    df_weather, result = etl_weather_to_doris(dt, doris_raw_adapter)
    logging.info("Weather ETL Success:")
except Exception as e:
    logging.info("Weather ETL Failed:", str(e))
prod_log = get_todays_production_logs(mysql_adapter, dt)
insert_production_logs_to_doris(prod_log, doris_raw_adapter)
mysql_adapter.close()
logging.info("grab data complete; start inject transform")
doris_raw_adapter.execute_query(f"""
...
""")
logging.info("start inject data mart")
doris_raw_adapter.execute_query(f"""
